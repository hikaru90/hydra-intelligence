<script lang="ts">
  ...
  let startTime = $state(subtractTime(new Date(), 10));
  ...
  const temperatureColor = $derived.by(() => scaleSequential(
    extent(tempChart, (d) => d.value) as [number, number],
    interpolateTurbo
  ));
...
<div class={className}>
  ...
  <div class="flex items-center justify-between">
    ...
    <button onclick={() => {refreshing = true; getData()}} class="text-emerald-500 p-2 rounded-md {refreshing ? 'animate-spin' : ''}">
